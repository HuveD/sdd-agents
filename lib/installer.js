...
class SddInstaller {
  ...
  async init(options = {}) {
    ...
    try {
      ...
      await this.createAgentDirectory();

      // 5. Copy .agent template files
      await this.copyAgentTemplates(options.force);

      // 6. Create docs directory structure
      await this.createDocsDirectory();

      // 7. Copy docs template files (deprecated, kept for backwards compatibility)
      await this.copyDocsTemplates(options.force);

      // 8. Copy docs specs files
      await this.copyDocsSpecs(options.force);

      // 9. Copy templates root files
      await this.copyTemplatesRootFiles(options.force);

      // 10. Save version (for first-time installation)
  ...
  async createAgentDirectory() {
    const agentDir = path.join(this.currentDir, '.agent');
    await fs.ensureDir(agentDir);
    await fs.ensureDir(path.join(agentDir, 'templates'));
    console.log('📁 .agent directory structure created');
  }

  async copyAgentTemplates(force = false) {
    const agentSourceDir = path.join(this.templatesDir, '.agent');
    const agentDestDir = path.join(this.currentDir, '.agent');

    if (!(await fs.pathExists(agentSourceDir))) {
      console.log('ℹ️  No .agent directory found in package, skipping');
      return;
    }

    await this.copyAgentRecursive(agentSourceDir, agentDestDir, force);
  }

  async copyAgentRecursive(sourceDir, destDir, force = false) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        await this.copyAgentRecursive(sourcePath, destPath, force);
      } else {
        const relativePath = path.relative(this.currentDir, destPath);

        if (await fs.pathExists(destPath) && !force) {
          console.log(`⚠️  ${relativePath} already exists (use --force to overwrite)`);
          continue;
        }
  ...
  async copyDocsTemplates(force = false) {
    ...
    if (!(await fs.pathExists(docsSource))) {
      console.log('ℹ️  No docs/templates directory found in package, skipping');
      return;
    }
    ...
    if (templateFiles.length === 0) {
      console.log('ℹ️  docs/templates directory is empty, skipping');
      return;
    }
